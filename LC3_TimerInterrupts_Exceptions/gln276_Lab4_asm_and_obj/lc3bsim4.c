...
void eval_micro_sequencer()
{
  ...
  */
  ...
  if (DEBUG_INT) { printf("____________________\nCURRENT CYCLE: %d\nCURRENT STATE: %d\n____________________\n", 
                   CYCLE_COUNT, 
                   CURRENT_LATCHES.STATE_NUMBER); }
}
  ...
  */
  ...
  for (i = GATE_PC; i <= GATE_SHF; i += 1)
  {
    ...
    if (lDriveCount == 1)
    {
      ...
      {
      case GATE_PC:
        if (DEBUG_INT) { printf("GATE_PC driven to bus, value %x\n", Gate_PC_Input); }
        BUS = Gate_PC_Input;
        break;
      case GATE_MDR:
        if (DEBUG_INT) { printf("GATE_MDR driven to bus, value %x\n", Gate_MDR_Input); }
        BUS = Gate_MDR_Input;
        break;
      case GATE_ALU:
        if (DEBUG_INT) { printf("GATE_ALU driven to bus, value %x\n", Gate_ALU_Input); }
        BUS = Gate_ALU_Input;
        break;
      case GATE_MARMUX:
        if (DEBUG_INT) { printf("GATE_MARMUX driven to bus, value %x\n", Gate_MARMUX_Input); }
        BUS = Gate_MARMUX_Input;
        break;
      case GATE_SHF:
        if (DEBUG_INT) { printf("GATE_SHF driven to bus, value %x\n", Gate_SHF_Input); }
        BUS = Gate_SHF_Input;
        break;
      }
      lDriveCount += 1;
  ...
  for (i = Gate_PSR; i <= Gate_IE_V; i += 1)
  {
    ...
    if (lDriveCount == 1)
    {
      ...
      {
      case Gate_PSR:
        if (DEBUG_INT) { printf("Gate_PSR driven to bus, value %x\n", Gate_PSR_Input); }
        BUS = Gate_PSR_Input;
        break;
      case Gate_USP_RES:
        if (DEBUG_INT) { printf("Gate_USP_RES driven to bus, value %x\n", Gate_USP_RES_Input); }
        BUS = Gate_USP_RES_Input;
        break;
      case Gate_SSP_RES:
        if (DEBUG_INT) { printf("Gate_SSP_RES driven to bus, value %x\n", GATE_SSP_RES_Input); }
        BUS = GATE_SSP_RES_Input;
        break;
      case Gate_SP:
        if (DEBUG_INT) { printf("Gate_SP driven to bus, value %x\n", GATE_SP_Input); }
        BUS = GATE_SP_Input;
        break;
      case Gate_IE_V:
        if (DEBUG_INT) { printf("Gate_IE_V driven to bus, value %x\n", GATE_IE_V_Input); }
        BUS = GATE_IE_V_Input;
        break;
      }
      lDriveCount += 1;
  ...
  {
    if (DEBUG_INT) { printf("GATE_DEC_PC driven to bus, value %x\n", Gate_DEC_PC_Input); }
  ...
  {
    BUS = 0;
    if (DEBUG_INT) { printf("Bus driven to 0\n"); }
  }
...
void updateUSequencer()
{
  ...
  if (GETCLD_E2(CURRENT_LATCHES.MICROINSTRUCTION) > 0)
  {
    if ((CYCLE_COUNT >= Int_Cycle) && (TIE == 0))
    {
      ...
      NEXT_LATCHES.STATE_NUMBER |= E2Bits;
      updateNextMicroinstruction();
      TIE = 1; 
    }
  }
  ...
  {
    if ((CURRENT_LATCHES.PSR & 0x8000) == 1)
    {
...
void latch_datapath_values()
{
  ...
  if (GetLD_TEMP_V(CURRENT_LATCHES.MICROINSTRUCTION) > 0)
  {
    ...
    if (DEBUG_INT) { printf("tempv loaded with : %x\n", NEXT_LATCHES.Temp_V); }
  }
}
